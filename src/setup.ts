...
function isCloneContext(): boolean {
  try {
    ...
    return pkg.name === "council-of-models-mcp";
  } catch {
    return false;
  }
}
...
function buildRegistrationArgs(): string[] {
  ...
  try {
    execFileSync("council-mcp", ["--help"], { stdio: "pipe", timeout: 3000 });
    return ["mcp", "add", "--scope", "user", "council-mcp", "--", "council-mcp"];
  } catch {
    const indexPath = join(__dirname, "index.js");
    return ["mcp", "add", "--scope", "user", "council-mcp", "--", "node", indexPath];
  }
}

function copyTemplates(targetDir: string): boolean {
  const councilSrc = join(EXAMPLES_DIR, "council.md");
  const refineSrc = join(EXAMPLES_DIR, "refine.md");

  if (!existsSync(councilSrc) || !existsSync(refineSrc)) {
    p.log.error(
      `Template files not found in ${EXAMPLES_DIR}.\n` +
        "If you installed via npm, try reinstalling: npm install -g council-of-models-mcp"
    );
    return false;
  }

  mkdirSync(targetDir, { recursive: true });
  copyFileSync(councilSrc, join(targetDir, "council.md"));
  copyFileSync(refineSrc, join(targetDir, "refine.md"));

  p.log.success(`Copied council.md -> ${targetDir}`);
...
async function main() {
  p.intro("Council of Models - Setup");
  ...
  if (inNpx) {
    p.log.warn(
      "Running from a temporary npx cache. MCP registration paths may break when the cache clears.\n" +
        "   For a stable installation, run: npm install -g council-of-models-mcp"
    );
  }
  ...
  if (claudeAvailable) {
    ...
    try {
      ...
      alreadyRegistered = mcpList.includes("council-mcp");
    } catch {
      // claude mcp list failed — skip registration check
    }

    if (alreadyRegistered) {
      p.log.success("council-mcp is already registered with Claude Code.");
    } else {
      const shouldRegister = await p.confirm({
        message: "Register council-mcp with Claude Code? (recommended)",
        initialValue: true,
      });
      ...
      if (shouldRegister) {
        ...
        try {
          ...
          s.stop("Registered! council-mcp is available in all Claude Code sessions.");
        } catch {
  ...
  const shouldSetupProject = await p.confirm({
    message: "Set up /council and /refine commands in a project now?",
    initialValue: true,
  });
  ...
  if (shouldSetupProject) {
    ...
    const councilExists = existsSync(join(targetDir, "council.md"));
    const refineExists = existsSync(join(targetDir, "refine.md"));

    if (councilExists || refineExists) {
      const overwrite = await p.confirm({
        message: "Existing /council or /refine commands found. Overwrite? (backups saved as .bak)",
        initialValue: false,
      });
      ...
      if (!overwrite) {
        ...
        if (councilExists) {
          copyFileSync(
            join(targetDir, "council.md"),
            join(targetDir, "council.md.bak")
          );
        }
        if (refineExists) {
          copyFileSync(
  ...
  if (shouldSetupProject) {
    ...
    nextSteps.push("Copy templates to a project later with: council-mcp-setup");
  }

  nextSteps.push("Try: /council to cross-validate an implementation plan");
