...
Look for `council-mcp` with `✓ Connected`.
...
# Find the templates:
# npm install: look in $(npm root -g)/council-of-models-mcp/examples/claude-commands/
...
Start a new Claude Code session in your project and run `/council`.

For tailored commands, also copy `setup-council.md` and run `/setup-council`.
